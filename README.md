...
## class
# classList, className
* `DOM.className = '추가클래스명'`:원하는 DOM 대상에 새로운 클래스명을 적용할 수 있다.
* className 속성 사용 시 대입연산자를 사용하여 클래스를 2개 이상 적용 시 마지막 클래스만 적용됨.
* 추가 클래스를 적용하고 싶은 경우는 복합연산자를 사용.
* 주의) 복합연산자로 연결하는 추가 클래스를 공백 없이 입력 시 기존 클래스의 이름이 변경되는 것으로 인식함.
* 우선순위를 주의해서 사용해야한다. 우선순위 높은 순 : inline(4) > id(3) > class(2) > tag(1)
# add
* `DOM.classList.add('추가클래스명')`:원하는 DOM대상에 새로운 클래스명을 적용할 수 있다.
* className과 다르게 공백 또는 복합연산자없이 메서드 구조로 실행되어 별도의 클래스등록이 가능하다.
* 동시에 2개 이상의 클래스를 콤마(,)로 구분하여 선언할 수 있다. (개수제한없음)
# remove
* `DOM.classList.remove('제거클래스명')`:원하는 DOM대상에 적용된 클래스명을 제거할 수 있다.
* 2개 이상의 클래스를 제거하고 싶은 경우에는 콤마로 구분하여 작성한다.
# toggle
* `DOM.classList.toggle('토글클래스명')`:원하는 DOM대상 클래스 적용 상태에 따라 추가/제거를 실행함
* 스위치처럼 반복적인 제어가 가능하다.
* 기존에 스타일이 많이 적용되어있을 경우, 속성 안에 !important;를 적용하여 우선순위를 주면 됨
# replace
* `DOM.classList.replac('기존','변경')`:기존 클래스를 다른 이름으로 변경함
* 기존 클래스가 2개 이상인 경우 replace로 설정한 기준 클래스만 변경되고 그 외 클래스는 변동되지 않는다.
# contains
* `DOM.classList.contains(클래스 유무 확인)`:클래스의 존재 유무를 확인
* true or false를 반환한다.
* 조건문에서 논리데이터를 뽑는 용도로 많이 사용.
* 클래스뿐만아니라 추가메서드,속성 등 뭐든지 가능하다.
* dom상태의 따라 자바스크립트 활용범위가 넓다.
* 참거짓을 뽑기 때문에, 조건문과 함께하는 함수